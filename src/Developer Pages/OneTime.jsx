...
import { Folder, Eye, Edit, Send } from "lucide-react";
...
const OneTime = () => {
  ...
  useEffect(() => {
    ...
    const fetchOneTimeProjects = async () => {
      try {
        const token = localStorage.getItem("token");
        const response = await axios.get(
          `${API_BASE}/api/newproject/projects/one-time`,
          {
            headers: {
  ...
  const handleSendUpdate = async () => {
    ...
    const newUpdate = {
      ...
      createdBy: {
        ...
        username: localStorage.getItem("username") || "You", // Fallback username
      },
    };
    ...
    try {
      ...
      // Refresh updates after successful post
      const response = await axios.get(
        `${API_BASE}/api/newproject/projects/${selectedProject._id}/updates`,
        {
          headers: {
  ...
  //     const token = localStorage.getItem("token");
  //     await axios.post(
  //       `http://localhost:5000/api/newproject/projects/${selectedProject._id}/updates`,
  //       { text: message },
  //       {
  //         headers: {
  //           Authorization: `Bearer ${token}`,
            ...
            <Card key={project._id} sx={{ mb: 3 }}>
              ...
              >
                <Box display="flex" alignItems="center">
                  <FolderIcon
                    style={{ color: "#ffbf00  ", fontSize: "6rem" }}
                  />
                  <Box ml={2}>
      ...
      >
        ...
        <DialogContent sx={{ flex: 1, overflow: "auto", p: 0 }}>
          {isLoadingUpdates ? (
            ...
            <List sx={{ p: 0 }}>
              {updates.map((update) => (
                <ListItem key={update._id || tempId} alignItems="flex-start">
                  <ListItemAvatar>
                    <Avatar
                      sx={{
