...
// import {
//   FolderDot, CheckCircle2, Clock, AlertTriangle, AlertCircle, 
//   Calendar, Eye, Trash2, Edit3, MessageSquare, ExternalLink,
//   Plus, X, ChevronDown, ChevronUp, ChevronRight, Loader2,
//   Briefcase, Send, Shield, Search, Badge , KanbanSquare 
// } from "lucide-react";
...
//       await axios.post(`${API_BASE}/api/tasks/${projectId}/${task._id}/complete`, {}, { headers: authHeaders() });
//       onTaskComplete(task._id, projectId);
//     } catch (err) { console.error(err); setCompleting(false); }
//   };
...
// // ── Skeleton Loader ───────────────────────────────────────────────────────────
// const SkeletonCard = () => (
//   <div className="neu-pressed rounded-xl p-4 mb-4 opacity-50 skeleton-pulse">
//     <div className="flex gap-3">
//       <div className="w-2 h-2 rounded-full bg-[#D1DCEB] mt-1 shrink-0" />
//       <div className="flex-1 space-y-3">
//         <div className="h-3 bg-[#D1DCEB] rounded w-3/4" />
//         <div className="h-2 bg-[#D1DCEB] rounded w-1/2" />
//         <div className="flex gap-2 pt-2">
//           <div className="h-6 w-16 bg-[#D1DCEB] rounded-md" />
//           <div className="h-6 w-16 bg-[#D1DCEB] rounded-md" />
//         </div>
//       </div>
//     </div>
...
//       const mine = all.filter(p => (p.assignedDeveloper || []).some(d => d.id && currentUserId && d.id.toString() === currentUserId.toString()));
...
//           tasksResult[p._id] = (tr.data || []).filter(t => t.status !== "Done" && t.assignedTo?.id?.toString() === currentUserId?.toString());
...
//           list.forEach(item => {
//             const isMine = item.completedBy?.id?.toString() === currentUserId?.toString() || item.completedBy?._id?.toString() === currentUserId?.toString() || item.completedBy?.username === currentUsername;
//             if (isMine) completedList.push({ ...item, projectName: p.projectName, projectId: p._id });
//           });
//         } catch (e) {}
//       }));
...
//     const list = [];
//     projects.forEach(p => { (tasks[p._id] || []).forEach(t => list.push({ ...t, projectId: p._id, projectName: p.projectName })); });
//     return list.sort((a, b) => {
...
//   return (
//     <div className="h-screen w-full flex flex-col overflow-hidden neu-base montserrat-regular text-[#1F2328] relative">
...
//                  <div className="w-6 h-6 border-4 border-[#D1DCEB] border-t-[#0969DA] rounded-full animate-spin" />
//                  <span className="text-xs font-bold text-[#656D76] uppercase tracking-wider">Loading Workspace...</span>
//               </div>
//             </div>
//           ) : activeTab === "projects" ? (
...
//           <ProjectKanban
//             open={kanbanOpen}
//             onClose={() => { setKanbanOpen(false); setKanbanProject(null); loadInitialData(true); }}
...
//         @keyframes pulse-skeleton { 0%, 100% { opacity: 0.4; } 50% { opacity: 0.15; } }
//         .skeleton-pulse { animation: pulse-skeleton 1.5s ease-in-out infinite; }
...
import {
  ...
  Calendar, Eye, Trash2, Edit3, MessageSquare, ExternalLink,
  Plus, X, ChevronDown, ChevronUp, ChevronRight, Loader2,
  Briefcase, Send, Shield, Search, Badge as BadgeIcon, KanbanSquare 
} from "lucide-react";
...
// ── Atom: UI Badge Component (The missing piece) ─────────────────────────────
    ...
    <div className={`neu-flat rounded-2xl mb-5 transition-all duration-300 ${closed ? "opacity-60" : "opacity-100"}`}>
      <div onClick={() => setExpanded(!expanded)} className={`p-5 flex items-center gap-4 cursor-pointer neu-action-btn rounded-2xl transition-colors ${expanded ? "neu-pressed" : ""}`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {(project.serviceType || []).map((s, i) => <Badge key={i} label={s} color={C.primary} />)}
            {/* THIS IS THE LINE YOU ASKED ABOUT - Displaying Subscription Level as a tag */}
            {project.subscriptionType && <Badge label={project.subscriptionType} color={C.textSec} />}
          </div>
        </div>
        <ChevronDown size={18} className={`text-[#656D76] transition-transform duration-300 ${expanded ? "rotate-180" : ""}`} />
