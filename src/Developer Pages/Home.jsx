...
//       onSuccess(true); // Pass true to trigger a silent refresh
//       onClose();
//     } catch (err) {
//       console.error(err);
...
//   // Reset lazy load counts when filters change
//   useEffect(() => { setPendingVisibleCount(10); }, [filterProject]);
//   useEffect(() => { setCompletedVisibleCount(10); }, [completedListProjectFilter, completedListTimeFilter, completedListCustomDates]);

//   // isSilent prevents the UI from turning into skeleton loaders during background updates
...
//         // Network Optimization: Process in chunks of 5 to avoid overwhelming the server
//         for (let i = 0; i < fetchedProjects.length; i += 5) {
//           const projectChunk = fetchedProjects.slice(i, i + 5);
          
//           await Promise.all(
//             projectChunk.map(async (p) => {
//               try {
...
//       await fetchDashboardData(true); // Call fetch silently
//     } catch (error) {
...
//                     {filteredCompletedTasksList.slice(0, completedVisibleCount).map((task) => (
//                       <li 
...
//                     ))}
//                   </ul>
//                   {completedVisibleCount < filteredCompletedTasksList.length && (
...
//                 <>
//                   <ul className="divide-y divide-gray-100">
//                     {filteredPending.slice(0, pendingVisibleCount).map((task) => {
...
//                     })}
//                   </ul>
//                   {pendingVisibleCount < filteredPending.length && (
//                     <div className="p-3 text-center border-t border-[#D0D7DE]">
//                       <button 
...
function App() {
  ...
  const fetchDashboardData = useCallback(async (isSilent = false) => {
    ...
    try {
      ...
      if (fetchedProjects.length > 0 && currentUserId) {
        // Chunked fetching to prevent overwhelming network
        for (let i = 0; i < fetchedProjects.length; i += 5) {
          const projectChunk = fetchedProjects.slice(i, i + 5);
          await Promise.all(
            projectChunk.map(async (p) => {
              try {
                ...
                console.error(`Failed to fetch tasks for ${p._id}`, err);
              }
            })
          );
        }
      ...
      // Save to Local Cache (Decreases load speed immensely on re-visits)
  ...
  const handleCompleteTask = async (taskId, projectId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.post(`${API_BASE}/api/tasks/${projectId}/${taskId}/complete`, {}, { headers: { Authorization: `Bearer ${token}` } });
      // Silent refresh to align exact DB IDs
      fetchDashboardData(true);
    } catch (error) {
      console.error("Failed to complete task", error);
      fetchDashboardData(true); // Revert on failure
    } finally {
      setCompletingTaskId(null);
    }
  };
  ...
  const globallyFilteredPending = useMemo(() => {
    ...
    // For pending tasks, filter by either Creation Date or Deadline to ensure they appear in relevant date views
    if (globalTimeFilter !== "All Time") {
      list = list.filter(t => 
        isDateInRange(t.createdAt) || 
        isDateInRange(t.deadline) || 
        (t.deadline && differenceInCalendarDays(new Date(t.deadline), new Date()) < 0) // Always show overdue regardless of date filter
      );
    }
    return list;
